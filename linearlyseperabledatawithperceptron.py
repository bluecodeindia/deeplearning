# -*- coding: utf-8 -*-
"""LinearlySeperableDataWithPerceptron.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1RitFDJ1BZWrMduXCAsQrHjHQUGzAigCo
"""
...
Folder=[File+"Group31/Classification/LS_Group31"]
...
# Decision boundary
def generate_values_boundary(data,weights1,weights2,weights3,ttl):
    min_x=min(data[:,1])
    max_x=max(data[:,1])
    min_y=min(data[:,2])
    max_y=max(data[:,2])
...
#     plt.legend()
    ...
    s=int(data.shape[0]/3)
    plt.scatter(data[:s,1],data[:s,2],marker='D',color='#EAA358',edgecolors='black')
    plt.scatter(data[s:2*s,1],data[s:2*s,2],marker='X',color='#65993a',edgecolors='black')
    plt.scatter(data[2*s:,1],data[2*s:,2],marker='o',color='#4486B7',edgecolors='black')
...
def test(test_data,test_y,weights1,weights2,weights3,log=1):   
    ...
    for i in range(test_data.shape[0]):
        ...
        if  predict1==0 and predict2==0:
            cls=0
        if  predict1==1 and predict3==1:
            cls=1
        if  predict3==0 and predict1==0:
            cls=2
...
weights1,weights2,weights3,error=network(red,green,blue,epoch=300,log=1)
...
generate_values_boundary(data,weights1,weights2,weights3,'Training')

test(data,y,weights1,weights2,weights3,log=1)

generate_values_boundary(valid_data,weights1,weights2,weights3,'Validation')

test(valid_data,valid_y,weights1,weights2,weights3,log=1)

generate_values_boundary(test_data,weights1,weights2,weights3,'Test')
